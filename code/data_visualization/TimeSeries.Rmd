...
colnames(df_agg)[3] <- 'Total Deposits'
```

```{r}
df_agg$`Total Deposits` <- df_agg$`Total Deposits` * 1000
```
...
a <- ggplot(df_agg, aes(x = `Date`)) + geom_bar(aes(y = `Total Deposits`, fill = `Bank Class`), stat = 'identity') + 
...
```{r}
decades_wide$top5 = decades_wide$top5 * 1000
decades_wide$next125 = decades_wide$next125 * 1000
decades_wide$others = decades_wide$others * 1000
```
