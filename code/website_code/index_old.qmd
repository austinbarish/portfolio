...
#### **Table of US bank assets, equity and liabilities per state of of 2022Q4** {#sec-interactive-table}

```{python}
#| echo: false
#| label: fig-interactive-table
#| fig-cap: "shows an interactive table with a dropdown allowing the selection of assets, equity, or liabilities."

IFrame(src='../website/plots/interactive_table.html', width=700, height=600)
...
In @fig-dot-and-heatmap, we take a deeper dive into what's going on. We can see in the heatmap layer that there are many places in the U.S. where banks aggregate; however, there is nothing that seems particularly special about South Dakota, Ohio, and North Carolina. However, upon further inspection, if we zoom into the plot, we can see that Citibank and Wells Fargo are in South Dakota, JP Morgan Chase in Ohio, and Bank of America in North Carolina. How can this be?
...
#### Temporal trend of the total domestic deposits in the US for the past 3 decades {#sec-temporal-assets}
...
#### Assets held by the top 4 banks in 2022 in comparison to all others {#sec-top-banks}
...
#### The distribution of commercial banks in the US by the assets in possession for Q42022 {#sec-top-commercial}
...
#### Breakdown of top 5 banks, the next 125 banks, and all other banks {#sec-grouped-top-banks}

```{python}
#| echo: false
#| label: fig-grouped-top-banks
#| fig-cap: "A grouped bar plot which shows the temporal trend of the distribution of assets"
...
## Bank deposits by state

#### **State bank deposits by bank** {#sec-state-bank-deposits}
...
#### **Top and Bottom 20 Histogram** {#sec-top-bottom-histogram}
...
#### **Quarters Line Graph** {#sec-quarters-line-graph}
...
#### **EQR and Assets: Small Counts** {#sec-eqr-and-assets}
...
In this project, we looked at the U.S. banking system through many different aspects. We observed the fact that assets of U.S. banks are concentrated predominantly in the states of South Dakota, Ohio, and North Carolina. We also analyzed potential reasons for this occurrence. We also dive deep into the analytics of how banks' assets in the U.S. are disproportionately concentrated in few institutions and finally consider the system's health.
